...
const CreditCard = ({ credit }) => {
  return (
    <Link to={`../../${credit.media_type}/${credit.id}`}>
      <div className="flex h-28 w-56 gap-2 rounded-md bg-accent-800 bg-opacity-40">
        {credit?.poster_path && (
          <img
            src={`https://image.tmdb.org/t/p/w342/${credit?.poster_path}`}
            alt={`credit - ${credit.title}/${credit.character}`}
            className=" w-20 rounded-l-md object-cover"
          />
        )}
        <div className="flex flex-col items-start gap-1 p-1">
          <span className="text-font-100">{credit.title || credit.name}</span>
          ...
          <span className="text-xs text-font-50">{credit.character}</span>
          <span className="text-xs text-font-50">
